...
test.describe('one-click start and replay', () => {
  test('first tier change and result replay use the correct scene without another Start', async ({ page }) => {
    const errors: string[] = [];
    ...
    await page.locator('#difficultyPicker [data-difficulty="bunny"]').click();
    ...
    await page.locator('#finishDifficultyPicker [data-difficulty="black"]').click();
    await page.locator('#restartButton').click();
    await waitForRun(page);
  ...
  test('blocked storage preserves a requested tier and refreshing cannot replay stale intent', async ({ page }) => {
    await page.addInitScript(() => {
      Storage.prototype.getItem = () => { throw new DOMException('Storage blocked', 'SecurityError'); };
      Storage.prototype.setItem = () => { throw new DOMException('Storage blocked', 'SecurityError'); };
    });
    await gotoGame(page, '?intro=off&seed=123');
    await page.locator('#difficultyPicker [data-difficulty="expert"]').click();
    await page.locator('#startGameButton').click();
    await waitForRun(page);
  ...
  test('a Start before scripts arrive completes once when they become ready', async ({ page }) => {
    let release = () => {};
