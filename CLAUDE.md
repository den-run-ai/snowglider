...
- `src/agents/` - Living-world agent layer (issue #366, Roadmap Finding 5): a thin, explicit entity layer for moving world entities — a **peer** of `src/scenery/*`, deliberately at feature scope (NOT a framework-wide ECS rewrite, which would fight the byte-identical baseline/seeded-PRNG invariants). `agents.ts` is the `AgentSystem` facade (`createAgents()`); built in `game/scene-setup.ts` **after** the collision arrays (next to `createScenery`), ticked cosmetic-only in the `game/main-loop.ts` render-frame zone (never the fixed physics substep), and disposed in `game/teardown.ts`. Holds the SAME six invariants as scenery (render-only, collision-neutral, physics-neutral, `Math.random`-stream-neutral, teardown-safe, cosmetic-tick-only): placement/motion draws from the seeded `makeAgentRng()` (with its own private xorshift for THREE uuid draws, distinct seed constant from scenery/trees) and every THREE construction is wrapped in `withPrivateThreeRandom()` (`agents-rng.ts`); the seed is keyed off the run tier via `agentsSeedFor()` (`agents-budget.ts`). PR 1 ships the seam + `wildlife.ts` — a small, purely-**background** roaming herd (one InstancedMesh reusing the ambient-life `MeshBasicMaterial+fog` program → 0 new shader programs, +1 geometry), frozen under reduced motion. Future PRs add interactive agents (chasers, rival ghosts) that **report** an outcome routed through a pure resolver (the `avalanche.checkBurial`/`resolveBurialOutcome` pattern) — the main loop applies game-over, never the agent. Headless-tested: `npm run test:agents` (facade/seam/wildlife invariants) and `npm run test:agents-rng` (RNG neutrality).
