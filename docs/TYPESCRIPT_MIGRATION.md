...
- [x] Rename the remaining `.js` → `.ts` module-by-module (course → camera → trees → controls → effects → snow → mountains → snowman, snowglider last; auth/scores in parallel); move JSDoc typedefs to real `interface`/`type` declarations. **Done** as the stacked PR chain 3.1–3.11 (issue #98): course+effects, camera, trees, controls, snow, mountains, snowman, auth+scores, snowglider, audio, and the boot/entry cluster (main, script-loader, start-menu). Every `src/**` module is now `.ts`; only the two classic boot scripts (`src/boot/firebase-bootstrap.js`, `src/boot/local-auth.js`) stay `.js` by design.
- [x] **Enable `strictNullChecks` first** — it's the single highest-value flag for this game (empty raycaster hits, optional mesh refs, `getTerrainHeight` before injection). **Done:** `strictNullChecks: true` in `tsconfig.json`. The ratchet fixed 114 real nullable seams across 11 files with no behavior change:
  - **Injected seams** typed as `T | null` and guarded at the entry: `CourseModule`'s `getTerrainHeight`/`scene`/`createSnowman`, snowglider's `avalanche` (`AvalancheSystem | null`) and `_shake` (`ShakeOffset | null`), and the auth/scores Firebase handles (`firebaseApp: FirebaseApp | null`, `currentUser: User | null`).
...
- [ ] Then ratchet the rest: `noImplicitAny` → `strictFunctionTypes` → full `"strict": true`. One flag per PR; never big-bang `strict`.
  ...
  - [x] **`useUnknownInCatchVariables`** — done. Catch-clause variables are now `unknown` instead of `any`; the 11 error-logging sites (auth, firebase-bootstrap, snowglider) that read `e.message`/`e.stack` cast via `e as Error` (a local `err` where `.stack` is also read). Behavior-identical to the old implicit-`any` reads. Full gate set green (browser 87/0).
  - [x] **Full `"strict": true`** — done (Phase 3 capstone). Flipped `strict: true` and removed the now-redundant individual flag lines from `tsconfig.json`; this additionally enabled the remaining zero-cost flags (`strictPropertyInitialization`, `noImplicitThis`, `alwaysStrict`) with no code changes. `tsc --noEmit` is fully strict and green; full gate set green (browser 87/0).
- [~] Replace the shared mutable globals (`scene`, `velocity`, `gameActive`, `avalancheTriggered`, `lastAvalancheZ`, `bestTime`…) with a typed `GameState` object passed through the loop, or typed module state. This is where `PlayerState` / `Gate` / `GamePhase` / `AvalancheState` become real types and refactors get safe.
  - **Pilot landed (PR 3.19):** a `GameState` interface + a single `state` object in `snowglider.ts` now hold the **avalanche run-state** (`avalanche` / `avalancheTriggered` / `lastAvalancheZ`), the cohesive subset chosen to establish the pattern at low risk. The `window.*` accessor proxy keeps the original bare handle names (browser suites unchanged) but reads/writes `state.*`; the two avalanche blocks bind a `const avalanche = state.avalanche;` local for stable narrowing across the method calls.
  - **Run/scoring folded in (PR 3.20):** the next cohesive subset — the run timer origin `startTime` and the persisted `bestTime` — moved off loose module-scoped `let`s into the same typed `state: GameState` object. Every read/write in `snowglider.ts` now goes through `state.startTime` / `state.bestTime`, and the `window.*` proxy backs the bare `startTime`/`bestTime` handles with `state.*` (so `browser-regression-tests.js`, which reassigns both, is untouched). `readStoredBestTime()` still runs at the same point in module eval, preserving its prune-invalid-entry side effect. Behavior-preserving.
  - **Typed `physics.ts` extracted (PR 3.21):** the per-frame player physics state — the shared `pos`/`velocity` objects plus the reassigned air/jump/auto-turn scalars (~11 aliased module `let`s) — now lives in one typed `PlayerState` object owned by a new `src/physics.ts` module. `physics.ts` is the thin stateful layer (`createPlayerState` / `resetPlayer` / `stepPlayer`) on top of the **unchanged** physics kernel in `snowman.ts` (`updateSnowman` / `resetSnowman`), so the physics-invariant harness — which imports `snowman.ts` directly — is unaffected and coasting stays byte-identical. The orchestrator keeps `pos`/`velocity` as by-identity aliases (they are mutated in place and handed by reference to course/camera/snow/snowman) and accesses the reassigned scalars through `player.*` (and so does the `window.*` proxy). snowman's call-contract types (`PlayerPos`, `PlanarVelocity`, `TerrainHeightFn`, `TerrainVecFn`, `SnowmanControls`, `TreePos`, `CameraManagerLike`, `ShowGameOverFn`, `UpdateResult`) are now `export`ed so `physics.ts` shares them. A new node unit test (`tests/physics-state-tests.js`, under `npm run test:physics`) locks the create/reset/step contract against the real modules.
  - **Run-loop lifecycle flags folded in (PR 3.22):** the next cohesive subset — the run-loop lifecycle flags `gameActive`, `animationRunning`, `gameInitialized` — moved off loose module-scoped `let`s into the same typed `state: GameState` object. Every read/write in `snowglider.ts` now goes through `state.*`, and the `window.*` proxy backs the bare `gameActive` handle with `state.gameActive` (so the browser suites — which auto-start a run by setting `gameActive = true` and the regression suite which saves/restores it — are untouched). Behavior-preserving.
  - The remaining mutable scalars (`scene`/`renderer`/`camera` handles, `lastTechnique`, `lastTime`) stay as strict-typed module state; fold further cohesive subsets into `GameState`/`physics.ts` the same way as the game grows.

**Exit criteria:** all `src` is `.ts`, `"strict": true`, no `any` in domain/logic modules (rendering may keep a few, documented). **Met** (via the stacked PR chain through PR 3.17): every `src/**` module is `.ts` (the two classic `boot/*.js` scripts stay JS by design), `tsconfig` is fully `"strict": true` and green in CI, and the remaining `any`s are confined to deliberate boot/test seams (the `window.*` handles in `types/globals.d.ts`, untyped Firestore `DocumentData` fields like `userRef`, and the legacy `AuthModule`/`ScoresModule` globals). The optional follow-ups below (typed `GameState`) are not exit-blocking.

**Post-strict tooling — `typescript-eslint` (PR 3.18):** `.ts` sources are now linted. `eslint.config.js` adds a `**/*.ts` block using the `typescript-eslint` parser + `recommended` rules (non-type-checked — `tsc --noEmit` stays the type gate). Severities mirror the JS block (`@typescript-eslint/no-unused-vars` off; `@typescript-eslint/no-explicit-any` downgraded to `warn` since the only `any`s left are documented boot/test seams). Enabling it surfaced 4 real `prefer-const` errors (never-reassigned `let`s — `pos`/`velocity`/`turnAmplitude` in snowglider, `firestoreIsAvailable` in scores), now fixed. `npm run lint` is green (0 errors, `any`-seam warnings only).
